...
productCounter.addEventListener('change', (e) => {
...
addBtn.addEventListener('click', () => {
    if (productCounter.value && productCounter.value <= inStock) {
        if (!getCookie(id)) {
            document.cookie = `${id}=${productCounter.value}; SameSite=None; Secure` 
        } else {
            document.cookie = `${id}=${parseInt(productCounter.value) + parseInt(getCookie(id))}; SameSite=None; Secure`
        }
        let temp = inStock - getCookie(id)
        if (temp) {
            inStockContainer.innerHTML = `In stock: ${temp}`
